...
\begin{minipage}[c]{0.30\textwidth}
 \centering
 \IfFileExists{ubalogo.jpg}{\includegraphics[width=4.5cm]{ubalogo.jpg}}{\fbox{\parbox[c][3cm][c]{4cm}{\centering UBa logo\\\texttt{ubalogo.jpg}}}}
\end{minipage}%
\hfill
\begin{minipage}[c]{0.30\textwidth}
 \centering
...
\hline
Elmessery et al.\ (2023) broiler pathological phenomena (RGB subset) & Visible health conditions & Primary public training data for distress classes & Verify licence/quantities; thermal subset not used \\
\hline
Zarrat Ehsan \& Mohtavipour (2024) Broiler-Net material & Inactivity, huddling behaviour & Supplementary behaviour cues & Behaviour transfer needs local data \\
\hline
Local GreenFarms Poultry capture & RGB images/video in situ & Domain adaptation; minority-class examples & Requires annotation effort and time \\
\hline
\end{tabularx}
\end{table}

In keeping with the limitations of visible-sign detection, the system uses project-facing labels that describe what is observable rather than what is diagnostically certain: \texttt{normal}, \texttt{inactive\_or\_lethargic}, \texttt{open\_beak\_distress}, \texttt{huddling}, \texttt{mobility\_abnormality}, \texttt{visible\_physical\_abnormality}, and \texttt{uncertain}. The inclusion of an explicit \texttt{uncertain} class reflects the literature's caution about overconfident classification and supports a workflow in which ambiguous cases are escalated for human review rather than acted upon automatically.
...
The DHT22 is validated by comparison against a reference thermometer and hygrometer across the expected operating range, and any systematic offset is recorded for correction. The metal-oxide gas sensor is given a warm-up period before readings are trusted, and a baseline is established in clean air; because the sensor is non-selective, its output is interpreted as a relative risk index rather than an absolute ammonia concentration, and sensor drift over time is acknowledged and periodically re-baselined. Calibration procedures and the reference instruments used are recorded so that the validation can be reproduced; the resulting calibration evidence (plots and comparison tables) is to be produced during implementation and is represented here by a placeholder (Figure~\ref{fig:calib}).

\pendingfig{figures/screenshots/p01_sensor_calibration.png}{5cm}{Sensor calibration: DHT22 readings against reference instruments}{fig:calib}
...
Hardware development proceeds iteratively (Iteration~1 of Table~\ref{tab:iterations}): a breadboard prototype is built and bench-tested against reference instruments; problems such as wiring faults or noisy readings are diagnosed and corrected; the design is then consolidated onto a more robust interconnection and housed in a protective enclosure; and finally the unit is installed in the poultry house for field validation. Photographic evidence of the prototype, wiring, enclosure, and installation will be produced during implementation and is represented here by placeholders (Figures~\ref{fig:breadboard} and \ref{fig:installed}).

\pendingfig{figures/photos/p02_breadboard_prototype.jpg}{5cm}{Breadboard prototype of the hardware unit}{fig:breadboard}
\pendingfig{figures/photos/p03_installed_prototype.jpg}{5cm}{Prototype installed in the poultry house}{fig:installed}
...
Public datasets are acquired from their original repositories, and their licences, quantities, class definitions, and file formats are verified against the source publications before use; only the RGB subset of the broiler pathological phenomena dataset is used, because the deployed system uses a standard webcam. No claims about dataset size or composition are made beyond those stated by the sources.
...
Exploratory data analysis characterises the assembled dataset before training: class distribution (to quantify imbalance), image-size distribution, bounding-box size and aspect-ratio distribution, and the prevalence of blur, occlusion, variable lighting, and high bird density. The findings guide augmentation and splitting decisions. The resulting plots are to be produced during implementation and are represented by placeholders (Figures~\ref{fig:classdist} and \ref{fig:bboxdist}).

\pendingfig{figures/screenshots/p04_class_distribution.png}{5cm}{Dataset class-distribution chart}{fig:classdist}
\pendingfig{figures/screenshots/p05_bbox_distribution.png}{5cm}{Bounding-box size and aspect-ratio distribution}{fig:bboxdist}
...
Training is conducted in Google Colab using its GPU runtime, which provides sufficient acceleration for transfer learning without local GPU hardware. The environment, dependencies, and runtime are recorded for reproducibility, and a placeholder represents the eventual environment screenshot (Figure~\ref{fig:colab}).

\pendingfig{figures/screenshots/p06_colab_environment.png}{5cm}{Google Colab GPU training environment}{fig:colab}
...
Models are evaluated on the held-out test set using the metrics defined in Section~3.15, with classification metrics applied to the classification stage and detection metrics (IoU, mAP) applied to the detector. Error analysis examines false positives and false negatives, since their operational costs differ: a false negative is a missed distress signal, while a false positive risks alert fatigue. Evaluation artefacts (training and validation curves, precision--recall curves, and the confusion matrix) are to be produced during implementation and are represented by placeholders (Figures~\ref{fig:losscurve}, \ref{fig:prcurve}, and \ref{fig:confmat}).

\pendingfig{figures/screenshots/p07_training_curves.png}{5cm}{Training and validation loss curves}{fig:losscurve}
\pendingfig{figures/screenshots/p08_pr_curve.png}{5cm}{Precision--recall curve of the detection model}{fig:prcurve}
\pendingfig{figures/screenshots/p09_confusion_matrix.png}{5cm}{Confusion matrix of the classification stage}{fig:confmat}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc01.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc02.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc03.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc04.pdf}\par}

\noindent\textbf{UC-05 Manage Pens.} \textit{Actor:} Farm Manager. \textit{Intent:} define pens within a farm and their capacities.

{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc05.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc06.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc07.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc08.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc09.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc10.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc11.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc12.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc13.pdf}\par}

\noindent\textbf{UC-14 Record Expense.} \textit{Actor:} Farm Manager. \textit{Intent:} record operational expenses for cost analysis.

{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc14.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc15.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc16.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc17.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc18.pdf}\par}

\noindent\textbf{UC-19 Acknowledge Alert.} \textit{Actor:} Farm Manager. \textit{Intent:} acknowledge an active warning/critical alert.

{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc19.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc20.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc21.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc22.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc23.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc24.pdf}\par}
...
{\centering \includegraphics[width=0.8\textwidth,height=4.3cm,keepaspectratio]{figures/usecase/uc25.pdf}\par}
...
\begin{figure}[tbp]\centering
 \IfFileExists{figures/generated/wireframe_login.pdf}{\includegraphics[width=0.78\textwidth]{figures/generated/wireframe_login.pdf}}{\fbox{\parbox[c][5cm][c]{0.78\textwidth}{\centering Wireframe: login}}}\\[3pt]
 {\footnotesize (a) Login screen}\\[10pt]
 \IfFileExists{figures/generated/wireframe_dashboard.pdf}{\includegraphics[width=0.92\textwidth]{figures/generated/wireframe_dashboard.pdf}}{\fbox{\parbox[c][5cm][c]{0.92\textwidth}{\centering Wireframe: dashboard}}}\\[3pt]
 {\footnotesize (b) Dashboard}
\caption{Wireframes of the authentication and dashboard screens}\label{fig:wf_auth}
\end{figure}

\begin{figure}[tbp]\centering
 \IfFileExists{figures/generated/wireframe_list.pdf}{\includegraphics[width=0.92\textwidth]{figures/generated/wireframe_list.pdf}}{\fbox{\parbox[c][5cm][c]{0.92\textwidth}{\centering Wireframe: list}}}\\[3pt]
 {\footnotesize (a) List / index screen}\\[10pt]
 \IfFileExists{figures/generated/wireframe_form.pdf}{\includegraphics[width=0.92\textwidth]{figures/generated/wireframe_form.pdf}}{\fbox{\parbox[c][5cm][c]{0.92\textwidth}{\centering Wireframe: form}}}\\[3pt]
...
The physical bench results for Objective One, the assembled circuit's measured sensor readings, the DHT22 and MQ-series wiring photographs, and the sensor calibration curve relating raw gas readings to a reference, were \emph{not} obtained within the prototype window and are therefore reported here as pending. The corresponding figures are represented in this chapter and in Section~3.8 by compile-safe placeholders that will render automatically once the photographs and calibration plot are captured. This deferral does not affect the control and buffering results reported above, which were obtained against the same code paths that the physical sensors will feed.
...
A configurable class-to-risk mapping was implemented as the pivotal contract linking the model to the application. Static analysis of the subsystem identified the mapping-resolution function as the most connected element of the entire machine-learning codebase, confirming its role as the central abstraction through which the six trained classes, \texttt{healthy}, \texttt{lethargic}, \texttt{open\_beak\_stress}, \texttt{diseased\_eye}, \texttt{slipped\_tendon}, and \texttt{pendulous\_crop}, are translated into web-application risk categories and alert severities. Automated tests confirmed that a known class resolved to its documented risk category and severity (for example, \texttt{slipped\_tendon} to a \emph{mobility abnormality} of \emph{critical} severity), that an unknown class resolved safely to an \emph{uncertain} outcome rather than an error, and that the inference payload assembled from a set of detections correctly selected the highest-confidence class, attached the mapped risk category, and always fell within the valid confidence range, including the boundary case of an empty detection set.

The trained-model results for Objective Two, the class-distribution and bounding-box plots from exploratory analysis, the training and validation loss curves, the precision, recall, $F_1$, and mean-average-precision figures, the confusion matrix, the sample correct and incorrect predictions, and the Raspberry Pi inference-latency benchmark, were \emph{not} produced, because the licensed dataset and the graphics-processing hardware required for training were outside the resources available for the prototype. These artefacts are, accordingly, reported as pending and are represented by compile-safe placeholders; the code paths that generate every one of them were implemented and unit-tested, so the artefacts can be produced by executing the pipeline on the dataset without further development.
...
The evaluation of the whole system against the metrics defined in Section~3.15 was therefore partial. The functional-correctness, security, and reliability properties that could be exercised in software were evaluated and met, as recorded above. The metrics that depend on the physical deployment and on a trained model, measured environmental accuracy against a reference instrument, model detection accuracy, on-device inference throughput, and a field usability assessment with the farm's staff, were not evaluated within the prototype window and are reported as pending, consistent with the deferrals noted under Objectives One and Two.

\section{Discussion}

\subsection{Interpretation for Objective One}

The results for the edge unit indicate that the control and reliability logic on which the hardware objective depends is sound independently of the physical sensors. The confirmation that actuation is edge-triggered, that a fan or heater event is emitted only when a threshold is crossed and not while the condition persists, matters because continuous relay switching would shorten actuator life and generate redundant records; the observed behaviour is the correct one for a low-cost prototype intended to run unattended. The demonstration that buffered records survive a restart and that duplicate submissions are suppressed addresses directly the intermittent-connectivity problem identified in Chapter~2 as a defining constraint of Sub-Saharan African deployments \textit{(Adenuga et al., 2023)}: a monitoring system that loses data during a power or network interruption, or that double-counts on reconnection, would forfeit the trust of the very smallholders it is meant to serve. Treating the metal-oxide gas reading as a validated but \emph{relative} ammonia-risk indicator, and rejecting physically impossible values at the boundary, is consistent with the position argued from the welfare literature in Chapter~2, that a cross-sensitive trend indicator honestly labelled as such is more useful to a smallholder than the complete omission of air quality that characterises many low-cost systems \textit{(Kristensen \& Wathes, 2000; Corkery et al., 2013)}. The principal limitation is that these findings were obtained in mock mode; the calibration relationship between the raw gas signal and a reference concentration, and the sensors' behaviour under the temperature and dust of an operating house, remain to be measured, and until they are, the environmental-accuracy claim is architectural rather than empirical.

\subsection{Interpretation for Objective Two}

The results for the vision subsystem show that the software scaffolding for a reproducible detection pipeline is complete and that the contract by which model output becomes an actionable alert is both implemented and tested, but that the model's empirical accuracy remains unestablished. The finding from static analysis that the class-to-risk mapping function is the most connected node in the machine-learning codebase is not merely a structural curiosity: it confirms that the design deliberately funnels every model class through a single, configurable translation point, which is what allows the trained model to be swapped, or the mapping to be re-tuned, without touching the application. This mirrors the edge-AI-for-poultry architecture advocated by \textit{Cakic et al.\ (2023)} and situates the present work within the same design tradition as the dataset on which it is based \textit{(Elmessery et al., 2023)}. The honest reporting of absent training metrics is a deliberate methodological choice rather than an omission: fabricating a confusion matrix or a mean-average-precision figure would violate the ethical standard the discipline requires and would mislead any reader attempting to judge the model's fitness. The limitation here is significant and is acknowledged plainly, without the licensed dataset and a graphics-processing unit the detector was not trained, so no claim is made about how accurately it distinguishes, say, \texttt{open\_beak\_stress} from \texttt{healthy}. What can be claimed is that once trained, the model's outputs will flow through a verified pipeline into the review interface and the alerting system, because that pathway was exercised end-to-end with representative inputs.
...
The study concludes that the control, validation, and offline-resilience logic required of the Raspberry Pi edge unit was successfully designed, implemented, and verified in software, and that the unit's architecture is sound, but that its empirical environmental accuracy remains to be established. The edge client was shown to evaluate temperature, humidity, and gas-risk readings against configurable thresholds, to actuate ventilation and heating on an edge-triggered basis, to buffer records durably across restarts, and to reject physically impossible inputs, each confirmed by automated test. The measurement of the sensors themselves against a reference instrument, and the calibration of the low-cost gas sensor as an ammonia-risk indicator, were not completed within the prototype window; the objective is therefore met at the level of design and control logic and remains open at the level of physical measurement.

\subsection{Conclusion on Objective Two}

The study concludes that the complete machine-learning pipeline for a reproducible, edge-deployable broiler-distress detector was implemented and unit-tested, and that the pivotal contract translating model classes into application risk categories and alerts was verified, but that no trained model and hence no accuracy metrics were produced. The pipeline provides dataset preparation with leakage-safe splitting, training on the lightweight YOLO11n architecture, evaluation, export to edge formats, and an inference service whose output conforms to the application's contract; the class-to-risk mapping was confirmed to be the central abstraction of the subsystem and to resolve known and unknown classes correctly. Because the licensed dataset and a graphics-processing unit lay outside the available resources, training was not carried out; the objective is therefore met at the level of the pipeline and the integration contract and remains open at the level of a trained, evaluated model.
